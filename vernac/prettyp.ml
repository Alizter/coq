...
let () = CErrors.register_handler @@ function
  | PrintNotationNotFound (entry, ntn_str) ->
      ...
      | Constrexpr.InConstrEntry -> "constr"
      | Constrexpr.InCustomEntry e -> e
      in
      Some Pp.(str "\"" ++ str ntn_str ++ str "\"" ++ spc ()
        ++ str "cannot be interpreted as a known notation in " ++ str entry_string ++ str " entry." ++ spc ()
        ++ str "Make sure that symbols are surrounded by spaces and that holes are explicitly denoted with \"_\".")
  | _ -> None
